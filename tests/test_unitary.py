...
class TestUnitary:
    ...
    def test_inverse_fixed(self):
        dimension = 2
        operation_name = 'U'
        unitary = Unitary(dimension, np.array([[np.exp(1j*np.pi/4), 0], [0, 1j]]), operation_name)
    ...
    def test_tensor(self):
        ...
        tensor_product = UnitaryDefinitions.sigmax().tensor(UnitaryDefinitions.sigmax())
    ...
    def test_rphi(self):
        ...
        for theta in theta_values:
            assert UnitaryDefinitions.rphi(theta, 0).close_to(UnitaryDefinitions.rx(theta))
            assert UnitaryDefinitions.rphi(theta, np.pi/2).close_to(UnitaryDefinitions.ry(theta))
    ...
    def test_display_name(self):
        ...
        unitary = Unitary(dimension, unitary.get_matrix(), operation_name, {parameter_name_1: (1.0, True)})
        ...
        unitary = Unitary(dimension, unitary.get_matrix(), operation_name, {parameter_name_1: (1.0, True), parameter_name_2: (2.0, False)})
    ...
    def test_gms(self):
        for num_qubits in [3,4,5]:
            ...
            assert u.left_multiply(u).left_multiply(u).left_multiply(u).close_to(Unitary.identity(u.get_dimension()))
...
class TestParameterizedUnitary:
    ...
    def test_parameterized_rotation(self):
        ...
        rotation = ParameterizedUnitary(dimension, rotation_matrix, parameters, operation_name)
        ...
        assert [p.get_parameter_name() in zero_rotation_unitary.get_display_name() for p in parameters]

        random_values = [p.random_value() for p in parameters]
        assert np.all([parameters[i].is_valid(r) for i,r in enumerate(random_values)])
        ...
        assert [p.get_parameter_name() in random_rotation_unitary.get_display_name() for p in parameters]
        assert random_rotation_unitary.left_multiply(random_rotation_unitary.inverse()).close_to(Unitary.identity(dimension))
    ...
    def test_parameterized_unitary_classmethods(self):
        ...
        assert zero_rotation_unitary.close_to(Unitary.identity(rotation_xy.get_dimension()))

        rotation_xyz = ParameterizedUnitaryDefinitions.rotation_xyz()
        zero_rotation_unitary = rotation_xyz.as_unitary([0, 0, 0])
        assert zero_rotation_unitary.close_to(Unitary.identity(rotation_xyz.get_dimension()))

        xx = ParameterizedUnitaryDefinitions.xx()
        xx_angle = 2*np.pi
        full_rotation_unitary = xx.as_unitary([xx_angle])
        assert full_rotation_unitary.close_to(Unitary.identity(xx.get_dimension()))

        angle_parameter_name = xx.get_parameters()[0].get_parameter_name()
        assert (xx_angle, True) == full_rotation_unitary.get_parameter_value(angle_parameter_name)

    def test_parameterized_unitary_time_evolution(self):
        sigmax = np.array([[0, 1], [1, 0]])
        t_min = -1.234
        t_max = 1.234
        time_evolution = ParameterizedUnitaryDefinitions.time_evolution(sigmax, t_min, t_max)
        zero_time_unitary = time_evolution.as_unitary([0])
        assert zero_time_unitary.close_to(Unitary.identity(time_evolution.get_dimension()))
        ...
        time_parameter_name = time_evolution.get_parameters()[0].get_parameter_name()
        assert (evolution_time, False) == forward_time_unitary.get_parameter_value(time_parameter_name)
        assert (-evolution_time, False) == backward_time_unitary.get_parameter_value(time_parameter_name)

        with pytest.raises(Exception):
            # switch ordering of t_min and t_max
            time_evolution = ParameterizedUnitaryDefinitions.time_evolution(sigmax, t_max, t_min)
...
class TestUnitarySequenceEntry:
    ...
    def test_cnot(self):
        ...
        system_dimension = 8
        full_unitary = entry.get_full_unitary(system_dimension)
        assert full_unitary.close_to(UnitaryDefinitions.cnot().tensor(Unitary.identity(2)))

        system_dimension = 16
        full_unitary = entry.get_full_unitary(system_dimension)
        assert full_unitary.close_to(UnitaryDefinitions.cnot().tensor(Unitary.identity(4)))
        assert full_unitary.left_multiply(full_unitary).close_to(Unitary.identity(system_dimension))
...
class TestUnitarySequence:
    ...
    def test_append_and_remove(self):
        ...
        sequence.append_first(UnitarySequenceEntry(UnitaryDefinitions.sigmax(), [0]))
        assert sequence.get_length() == 1
        assert sequence.product().close_to(UnitaryDefinitions.sigmax())

        sequence.append_last(UnitarySequenceEntry(UnitaryDefinitions.sigmay(), [0]))
        assert sequence.get_length() == 2
        assert sequence.product().close_to(UnitaryDefinitions.sigmaz())

        sequence.append_first(UnitarySequenceEntry(UnitaryDefinitions.sigmaz(), [0]))
    ...
    def test_undo(self):
        ...
        sequence.append_first(UnitarySequenceEntry(UnitaryDefinitions.sigmay(), [0]))
        sequence.append_first(UnitarySequenceEntry(UnitaryDefinitions.sigmay(), [0]))
        assert sequence.get_length() == 2
        assert sequence.product().close_to(identity)
    ...
    def test_combine(self):
        ...
        sequence_2 = UnitarySequence(dimension, [UnitarySequenceEntry(UnitaryDefinitions.sigmay(), [0])])

        combined_sequence = UnitarySequence.combine(sequence_1, sequence_2)
        assert combined_sequence.get_length() == (sequence_1.get_length() + sequence_2.get_length())
        assert combined_sequence.product().close_to(sequence_1.product().left_multiply(sequence_2.product()))
